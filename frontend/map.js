...
require([
        ...
        try {
            ...
            if (result.routeResults && result.routeResults.length > 0) {
                ...
                const routeSymbol = new SimpleLineSymbol({
                    color: [30, 136, 229],  // Blue theme color (#1e88e5)
                    width: 4
                });
    ...
    function addIncidentToMap(incident) {
        ...
        // Different styles for different incident types
        let color, outlineColor, popupIcon, markerStyle;
        if (incident.type_name === 'POLICE') {
            color = [30, 136, 229, 0.9]; // Blue
            outlineColor = [255, 255, 255];
            popupIcon = '🚔';
            markerStyle = 'diamond';
        } else if (incident.type_name === 'ACCIDENT') {
        ...
        // Use SimpleMarkerSymbol with distinctive shapes
        const markerSymbol = {
            type: "simple-marker",
            style: markerStyle,
            color: color,
            ...
            if (data.reports && data.reports.length > 0) {
                data.reports.forEach(report => {
                    ...
                    if (!knownReportIds.has(report.id)) {
                        console.log('New report detected:', report);
                        addIncidentToMap(report);
                        knownReportIds.add(report.id);
                        
                        // Show notification for new reports from other users
                        if (report.user_id !== user.id) {
                            const typeLabel = report.type_name === 'POLICE' ? '🚔 Police' : '🚗 Accident';
                            showToast(`New ${typeLabel} reported nearby!`, 'success');
                        }
                    }
                });
            }
    ...
    // Load incidents and start polling for real-time updates
    async function loadIncidentsAndStartPolling() {
        try {
            const response = await fetch('http://localhost:5000/api/reports', {
                headers: {
                    'Authorization': `Bearer ${token}`
            ...
            if (!response.ok) {
                ...
                // Still start polling even if no initial incidents
                setInterval(pollForNewReports, 5000);
                return;
            }
            ...
            // Add incidents to map and track their IDs
            if (data.reports && data.reports.length > 0) {
                data.reports.forEach(incident => {
                    addIncidentToMap(incident);
                    knownReportIds.add(incident.id);
                });
            }
            
            // Start polling for new reports every 5 seconds
            setInterval(pollForNewReports, 5000);
            console.log('Started real-time polling for new reports');
    ...
    const closeReportDropdown = document.getElementById('closeReportDropdown');
    const reportOptions = document.querySelectorAll('.report-option');
    
    let pendingReportLocation = null;
    
    // Close dropdown
    closeReportDropdown.addEventListener('click', () => {
        hideReportDropdown();
    });
    ...
    // Show dropdown at specific screen position
    function showReportDropdown(screenX, screenY, mapPoint) {
        pendingReportLocation = {
            longitude: mapPoint.longitude,
            latitude: mapPoint.latitude
        };
        
        // Get the map container's position to calculate fixed position
        const mapContainer = document.getElementById('mapDiv');
        const mapRect = mapContainer.getBoundingClientRect();
        
        // Calculate position relative to the viewport (fixed positioning)
        let posX = mapRect.left + screenX;
        let posY = mapRect.top + screenY;
        
        // Show dropdown first to get its dimensions
        reportDropdown.style.visibility = 'hidden';
        reportDropdown.classList.add('active');
        
        const dropdownRect = reportDropdown.getBoundingClientRect();
        const dropdownWidth = dropdownRect.width;
        const dropdownHeight = dropdownRect.height;
        
        // Center horizontally on click point
        posX = posX - (dropdownWidth / 2);
        
        // Position above the click point
        posY = posY - dropdownHeight - 15;
        
        // Keep within viewport bounds
        if (posX < 10) posX = 10;
        if (posX + dropdownWidth > window.innerWidth - 10) {
            posX = window.innerWidth - dropdownWidth - 10;
        }
        if (posY < 10) {
            // If not enough room above, show below the click point
            posY = mapRect.top + screenY + 15;
        }
        
        // Apply position
        reportDropdown.style.left = posX + 'px';
        reportDropdown.style.top = posY + 'px';
        reportDropdown.style.visibility = 'visible';
        
        console.log("Showing dropdown at:", posX, posY);
    }
    
    // Handle report option selection
    reportOptions.forEach(option => {
        option.addEventListener('click', async () => {
            const reportType = option.dataset.type;
            
            if (!pendingReportLocation) {
                console.error('No location set for report');
                return;
            }
            
            // Disable buttons while submitting
            reportOptions.forEach(opt => opt.disabled = true);
            ...
            try {
                const response = await fetch('http://localhost:5000/api/reports', {
                    ...
                    body: JSON.stringify({
                        latitude: pendingReportLocation.latitude,
                        longitude: pendingReportLocation.longitude,
                        type: reportType,
                        description: ''
                    })
                });
                ...
                if (data.success) {
                    ...
                    addIncidentToMap(data.report);
                    
                    // Also track it so we don't show it again from polling
                    knownReportIds.add(data.report.id);
                    
                    // Show success toast
                    const typeLabel = reportType === 'POLICE' ? '🚔 Police' : '🚗 Accident';
                    showToast(`${typeLabel} reported successfully!`, 'success');
                    
                    console.log('Report created:', data.report);
                } else {
                    showToast(data.message || 'Failed to create report', 'error');
                }
            } catch (error) {
                console.error('Error creating report:', error);
                showToast('Failed to report. Please try again.', 'error');
            } finally {
                // Re-enable buttons and hide dropdown
                reportOptions.forEach(opt => opt.disabled = false);
                hideReportDropdown();
            }
        });
    });
    ...
    view.on("click", function(event) {
        ...
        view.hitTest(event).then(function(response) {
            const graphic = response.results.find(result => 
                result.graphic && result.graphic.layer === incidentsLayer
            );
            
            if (graphic) {
                // Clicked on existing incident, let popup handle it
                return;
            }
            
            // Show report dropdown at click location
            console.log("Clicked at:", event.mapPoint.longitude, event.mapPoint.latitude);
            showReportDropdown(event.screenPoint.x, event.screenPoint.y, event.mapPoint);
        });
    });
    
    // Close dropdown when clicking elsewhere on map
    document.addEventListener('click', (e) => {
        if (!reportDropdown.contains(e.target) && !e.target.closest('#mapDiv')) {
            hideReportDropdown();
        }
    });
});
